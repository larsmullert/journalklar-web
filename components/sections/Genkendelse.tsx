        ...
        <div className="mt-14 max-w-[780px]">
          {problemer.map(({ label, tekst }, i) => (
            <div key={label} className={`py-9 ${i < problemer.length - 1 ? "border-b border-sand" : ""}`}>
              <div className="font-sans text-[11px] font-medium tracking-[0.14em] uppercase text-muted mb-4">
                {label}
              </div>
              <p className="font-sans text-[15px] font-light text-[#505050] leading-[1.75] mb-0">
                {tekst}
              </p>
            </div>
          ))}
